...
task count_reads {
  ...
  command <<<
    ...
    do
      ...
      /usr/local/bin/samtools view -b -f 0x2 -F 0x400 -q 20 -T ~{refFasta} ~{Bam} ${Chrom} | /usr/local/bin/intersectBed -sorted -nobuf -c -bed -b stdin -a ~{tmp}/${Chrom}.windows.bed > ${Chrom}.counts.bed &
    done;
    wait
  >>>
...
task annovar {
  ...
  command <<<
    ...
    if zgrep -v "^#" ~{combine_variants_vcf} | grep -q "."; then
      ...
      echo "Combine variants file is empty"
    fi
    if zgrep -v "^#" ~{annotate_variants_vcf} | grep -q "."; then
      ...
      echo "Annotate variants file is empty"
    fi
  >>>
  ...
  output{
    File? annovar_CombVar_vcf = glob("${annovar_CombVar_out}.${genome}_multianno.vcf")[0]
    File? annovar_CombVar_table = glob("${annovar_CombVar_out}.${genome}_multianno.txt")[0]
    File? annovar_AnnotVar_vcf = glob("${annovar_AnnotVar_out}.${genome}_multianno.vcf")[0]  # Optional output
    File? annovar_AnnotVar_table = glob("${annovar_AnnotVar_out}.${genome}_multianno.txt")[0]  # Optional output
