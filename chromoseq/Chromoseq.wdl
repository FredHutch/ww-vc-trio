...
task cov_qc {
  ...
  runtime {
    docker: "ghcr.io/sanaz01/cov_qc:latest"
    cpu: "4"
    memory: "32 G"
    job_group: jobGroup
  }
...
task run_manta {
  ...
  command <<<
    set -eo pipefail && \
    python2.7 /usr/local/src/manta/bin/configManta.py --config=${Config} --tumorBam=${Bam} --referenceFasta=${Reference} --runDir=manta --callRegions=${ReferenceBED} --outputContig && \
    python2.7 ./manta/runWorkflow.py -m local -q research-hpc -j 4 -g 32 && \
    zcat ./manta/results/variants/tumorSV.vcf.gz | /bin/sed 's/DUP:TANDEM/DUP/g' > fixed.vcf && \
    ${duphold_static} -v fixed.vcf -b ${Bam} -f ${Reference} -t 4 -o ${Name}.tumorSV.vcf && \
    /opt/htslib/bin/bgzip ${Name}.tumorSV.vcf && \
    /opt/htslib/bin/tabix ${Name}.tumorSV.vcf.gz
  >>>
  runtime {
    docker: "ghcr.io/sanaz01/manta:latest"
    cpu: "4"
    memory: "32 G"
    job_group: jobGroup
  }
...
task count_reads {
  ...
  command {
    set -eo pipefail && \
    (bedtools makewindows -b ${ReferenceBED} -w 500000 | \
    awk -v OFS="\t" -v C="${Chrom}" '$1==C && NF==3' > ${tmp}/${Chrom}.windows.bed) && \
    samtools view -b -f 0x2 -F 0x400 -q 20 -T ${refFasta} ${Bam} ${Chrom} | \
    intersectBed -sorted -nobuf -c -bed -b stdin -a ${tmp}/${Chrom}.windows.bed > ${Chrom}.counts.bed
  }

  runtime {
    docker: "ghcr.io/getwilds/samtools"
    cpu: "1"
    memory: "8 G"
    job_group: jobGroup
  }
...
task run_varscan {
  ...
  command <<<
    ...
    --min-var-freq ${default="0.02" MinFreq} --p-value ${default="0.01" pvalsnv} --output-vcf | /opt/htslib/bin/bgzip -c > ${Name}.snv.vcf.gz && \
    /opt/htslib/bin/tabix ${Name}.snv.vcf.gz && \
    ...
    --min-var-freq ${default="0.02" MinFreq} --p-value ${default="0.1" pvalindel} --output-vcf | /opt/htslib/bin/bgzip -c > ${Name}.indel.vcf.gz && \
    /opt/htslib/bin/tabix ${Name}.indel.vcf.gz
  >>>
  
  runtime {
    docker: "ghcr.io/sanaz01/varscan2:latest"
    cpu: "4"
    memory: "16 G"
    job_group: jobGroup
  }
...
task run_pindel_region {
  ...
  command <<<
    ...
    /opt/pindel-0.3-update/pindel -f ${refFasta} -p ${tmp}/in.pindel -c ${Reg} -o ${tmp}/out.pindel && \
    /opt/pindel-0.3-update/pindel2vcf -P ${tmp}/out.pindel -G -r ${refFasta} -e ${default=3 MinReads} -R ${default="hg38" genome} -d ${default="hg38" genome} -v ${tmp}/pindel.vcf && \
    sed 's/END=[0-9]*\;//' ${tmp}/pindel.vcf | /opt/htslib/bin/bgzip -c > ${Name}.pindel.vcf.gz && \
    /opt/htslib/bin/tabix ${Name}.pindel.vcf.gz
  >>>
  
  runtime {
    docker: "ghcr.io/sanaz01/pindel:latest"
    cpu: "2"
    memory: "16 G"
    job_group: jobGroup
  }
...
task run_octopus {    # Platypus is a different variant caller
  String Bam
  String BamIndex
  String CoverageBed
  String? DocmVcf
  ...
  command <<<
    ...
    /bin/sed 's/VCFv4.3/VCFv4.1/' "${Name}.vcf" > "${Name}.octopus.vcf"     
  >>>
  ...
  output {
    File octopus_vcf_file = "${Name}.octopus.vcf"
  }
}
...
task combine_variants {
  ...
  command {
    ...
    bcftools norm -m- -f ${refFasta} -O z > ${tmp}/combined.vcf.gz && \
    tabix -p vcf ${tmp}/combined.vcf.gz && \
    python3 /fh/fast/paguirigan_a/sanaz/Research/VariantCalling/ww-vc-trio_sanaz/chromoseq/workflow_scripts/addReadCountsToVcfCRAM3.py -n ${MinReads} -v ${MinVAF} -r ${refFasta} ${tmp}/combined.vcf.gz ${Bam} ${Name} | \
    /opt/htslib/bin/bgzip -c > ${Name}.combined_tagged.vcf.gz && \
    tabix -p vcf ${Name}.combined_tagged.vcf.gz
  }
  runtime {
    docker: "ghcr.io/sanaz01/combine_variants"
    cpu: "1"
    memory: "10 G"
    job_group: jobGroup
  }
...
task annotate_variants {
  ...
  command {
    set -eo pipefail && \
    vep --format vcf --vcf --fasta ${refFasta} --hgvs --symbol --term SO --per_gene -o ${Name}.annotated.vcf \
    ...
    filter_vep -i ${Name}.annotated.vcf.gz --format vcf -o ${Name}.annotated_filtered.vcf --filter "(MAX_AF < ${default='0.001' maxAF} or not MAX_AF) or MYELOSEQ_TCGA_AC or MYELOSEQ_MDS_AC" && \
    bgzip -c ${Name}.annotated_filtered.vcf > ${Name}.annotated_filtered.vcf.gz && \
    tabix -p vcf ${Name}.annotated_filtered.vcf.gz
  }
  runtime {
...
task annotate_svs {
  ...
  command {
    set -eo pipefail && \
    perl /fh/fast/paguirigan_a/sanaz/Research/VariantCalling/ww-vc-trio_sanaz/chromoseq/workflow_scripts/ichorToVCF.pl -g ${gender} -minsize ${minCNAsize} \
    ...
    perl /fh/fast/paguirigan_a/sanaz/Research/VariantCalling/ww-vc-trio_sanaz/chromoseq/workflow_scripts/FilterManta.pl -a ${minCNAabund} -r ${refFasta} -k ${Translocations} ${Vcf} filtered.vcf && \
    ...
    /opt/conda/envs/python2/bin/python /fh/fast/paguirigan_a/sanaz/Research/VariantCalling/ww-vc-trio_sanaz/chromoseq/workflow_scripts/convertInversion.py /usr/local/bin/samtools ${refFasta} svs.vcf | bgzip -c > svs.vcf.gz && \
    vep --format vcf --vcf --fasta ${refFasta} --per_gene --symbol --term SO -o ${Name}.svs_annotated.vcf -i svs.vcf.gz --custom ${Cytobands},cytobands,bed --offline --cache --dir ${Vepcache} && \
...
task make_report {
  ...
  command <<<
    ...
    /opt/conda/bin/python /fh/fast/paguirigan_a/sanaz/Research/VariantCalling/ww-vc-trio_sanaz/chromoseq/workflow_scripts/make_report3.py ${Name} ${GeneVCF} ${SVVCF} ${KnownGenes} "qc.txt" ${GeneQC} ${SVQC} > "${Name}.chromoseq.txt"
  >>>
...
task gather_files {
  ...
  runtime {
    docker_image: "ubuntu:noble-20240114"
  }
  output {
    String done = stdout()
  }
